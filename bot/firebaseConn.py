...
class Firebase:
   ...
   def updateBotOrder(self, botId, clientOid, orderId, record):
      try:
         record['__lastUpdate'] = date_to_milliseconds(datetime.utcnow().isoformat())
         save = json.dumps(record)
   ...
   def getRecord(self,table,id):
      snapshots = self.firestore_db.collection('botrun').document(USERID).collection(table).doc(id).get()
      return snapshots
   ...
   def getActiveBots2(self):
      ret = []
      ref = self.firestore_db.collection('cryptoBot').document(USERID).collection("bots")
      query = ref.where(u'active', u'==', True)
      try:
         bots = query.get()
         if len(bots) > 0:
   ...
   def deleteOldLogs(self,daysBack=2):
      ref = self.firestore_db.collection('botrun').document(USERID).collection("liveBotLog")
