...
from firebaseConn import Firebase
...
from secrets_API_KEYS import TELEGRAM_BOT_KEY,TELEGRAM_CHAT_ID
LOG_INFO = 0
LOG_WARN = 5
LOG_ERROR= 10
LOG_DEBUG= 99
...
def addOrderToFB(bot,clientOId,orderCreated):
    #record = bot['id']
    #db.updateBotOrder(botId=bot['id'], clientOId=clientOId, orderCreated='')
    return

def updateBot(bot):
    db.update(table='bots', record=bot, id=bot['id'])
    return


def updateCryptoBot(bot):
    db.update(table='cryptoBot', record=bot, id=bot['id'])
    return

def updateBotItems(botId,gridItem):
    db.update(table='bots', id=botId, subRecordId=gridItem['id'], subrecordCollection='gridItems', subRecord=gridItem)
    return
def updateBotItemsOrders(botId,gridItemId,orderId,record):
    db.updateBotOrder(botId, gridItemId, orderId, record)
    return
...
def fblog(text='', type=LOG_INFO, recordType='message', logTable='livebot', details={}, fromException=False, asset=''):
    log = {}
    log["runID"]    = ""
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                log['detail']   = {}
    try:
        db.update(record=log, table='liveBotLog' , id=str(log['date']))
...
def logMessage( type, message, asset='',log_level = LOG_DEBUG):
    if type < log_level:
        messageLog = f'[{getLogType(type)}][{asset}]\n{message}'
        sendTelegramMessage(messageLog)

        if log_level == LOG_DEBUG:
            messageLog  = f'[{getLogType(type)}][{asset}][{datetime.utcnow().isoformat()}]\n{message}'
            print(messageLog)
        try:
            fblog(text=message,logTable='liveBotLog' ,type=type, asset=asset)
        except Exception as e:
